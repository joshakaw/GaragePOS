...
Installing `better-sqlite3` requires you to install in both renderer `/` and main app `/app` and run `npx electron-rebuild -f -w better-sqlite3` on both. Electron is special because it requires you to rebuild it specially for some packages, it seems, unlike a normal NodeJS project.
